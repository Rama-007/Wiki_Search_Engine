...
class wikihandler(ContentHandler):
	...
	def invertedindex(self,doc_id,titles,bodytext,infobox,categories,external_links,references):
		...
		#print(type(titles),type(bodytext),type(infobox),type(categories),type(external_links),type(references))
	...
	def endElement(self,name):
		...
		if(name=="id"):
			...
			global fp
			print(str(cont)+"-"+self.title.encode('utf-8'))
			fp.write(str(cont)+"-"+self.title.encode('utf-8')+'\n')
			self.title_flag=0
			self.title=""
		elif(name=="text"):
